  ...
  template: `
    <div class="flex flex-column card px-2 md:px-4 py-4 md:py-4 w-full h-full">
      <div class="px-2">
        ...
        <p class="text-base text-right text-green-400">Shipping: {{ shipping | currency: "ARS" : "symbol" }}</p>
      ...
      <div class="px-2">
        <div class="flex justify-content-around">
          <div>
            ...
            <label for="priceSpecial" class="ml-2"> Special price </label>
          </div>
          <div>
            <p-radioButton name="priceList" value="priceList" [(ngModel)]="price" inputId="priceList" />
            <label for="priceList" class="ml-2"> List price </label>
          </div>
        </div>
        <p class="mb-0">
          {{
            price === "priceSpecial"
              ? "Paying by bank deposit or transfer."
              : "Paying by credit or debit card, up to 12 interest-free installments."
          }}
        </p>
      </div>

      <p-divider />
      <p-button styleClass="w-full" label="Proceed to payment" routerLink="/checkout" [disabled]="!data().length" />
    </div>
  `,
